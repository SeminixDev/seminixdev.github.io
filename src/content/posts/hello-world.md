...
This site is built with [Astro](https://astro.build) — a static site generator that compiles everything to plain HTML at build time. Blog posts like this one are written in Markdown. No JavaScript required to read a post; interactive bits (the theme toggle, image carousel, lightbox) are small vanilla JS scripts.
